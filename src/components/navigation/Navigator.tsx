...
const Navigator: React.FC<{ routes: RouteType[] }> = ({ routes }) => {
    ...
    function getCartTab(): ReactNode {
        return routes.filter(route => route.type == 'cart').map(r =>
            <Tab
                component={NavLink}
                to={r.to}
                key={r.label}
                icon={<Badge color="error" anchorOrigin={{
                    vertical: 'bottom',
                    horizontal: 'left',
                }}>
                    {GetIcon(r.icon!)}
                ...
                pages.sub.length ?
                    <>
                        ...
                        <ListItemButton onClick={() => closeDrawer()}>
                            <Tab component={NavLink} to={pages.to} label={`Show all form ${pages.name}`} key={pages.name} />
                        </ListItemButton>
